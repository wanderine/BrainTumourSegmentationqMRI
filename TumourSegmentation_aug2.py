...
from utilities import load_NON_GD
...
class unet(object):
    ...
    def train_with_aug(self, im_gen_train, gt_gen_train, im_gen_valid, gt_gen_valid, nEpochs):       
        ...
        for e in range(nEpochs):
            ...
            #    Xb_train_temp, Yb_train_temp = im_gen_train.next(), gt_gen_train.next()
            #    Xb_valid_temp, Yb_valid_temp = im_gen_valid.next(), gt_gen_valid.next()
            #    Xb_train = np.concatenate((Xb_train,Xb_train_temp),axis=0)
            #    Yb_train = np.concatenate((Yb_train,Yb_train_temp),axis=0)
            #    Xb_valid = np.concatenate((Xb_valid,Xb_valid_temp),axis=0)
            #    Yb_valid = np.concatenate((Yb_valid,Yb_valid_temp),axis=0)
